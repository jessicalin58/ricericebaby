window.onload = function () {
	...
	var doStuff = function () {
		...
		// Convert image to a mask of the rice
		cv.cvtColor(image, image, cv.COLOR_RGBA2GRAY);
		cv.GaussianBlur(image, image, { height: 11, width: 11}, 0, 0, cv.BORDER_DEFAULT);
		cv.threshold(image, image, 165, 255, cv.THRESH_BINARY);

		// Find blobs/contours of rice in the image
		let contours = new cv.MatVector();
		const hierarchy = new cv.Mat();
		cv.findContours(image, contours, hierarchy, cv.RETR_CCOMP, cv.CHAIN_APPROX_SIMPLE, new cv.Point());
		hierarchy.delete();

		// Summation of areas occupied by rice and identify blob positions
		let rice_area = 0;
		let areas = [];
		...
		// Transform area into grain count
		let grain_count =  Math.floor(rice_area * 0.015) + " rice grains, with 100% confidence. [0%] margin of error.";

		// Trace a line between a blob of rice area and its nearest blob within a distance
		for (let i = 0; i < areas.length; i++) {
			...
			if (closest[0] != -1 && closest[1] != -1) {
				...
				cv.line(orig, p1, p2, color, 2);
			}
		}

		// Set seed and get some puns
